<template>
	<section>
		...
		<div class="br_black-3 br_solid br_1 bg_white">
			<header
				class="flex flex_row br_black-2 br_solid br-b_1 justify_center"
			>
				<h4 class="flex_auto self_center p-l_4 m_0">
					New Presentation
				</h4>
				<Btn :state="'empty'" @onClick="removePresentation()">remove</Btn>
			</header>
				...
				<InputTextAndButton :actionLabel="'Add Attachment'">
					...
					<template v-slot:buttonLabel>
						<i
							...
							"
						></i>
						<span class="display_none inline:md">Attach File</span>
					</template>
				</InputTextAndButton>
				<input-text
					icon="fa-stopwatch"
			...
			<header
				class="flex flex_row br_black-3 br_solid  br-t_1 justify_center p-y_3 p-r_3"
			>
            ...
            <ListLoader :list='faculty'>
               <template v-slot:listEmpty >
				...
				class="flex flex_row br_black-2 br_solid br-b_1 justify_center"
			>
				<i class="fas fa-grip-vertical self_center c_black-3 h:c_primary p-y_4 p-x_3 br-r_1 br_solid br_black-2 flex_none"></i>
                <h4 class="flex_auto self_center p-l_4 m_0">
					Faculty
				</h4>
				<Btn :state="'empty'" @onClick='faculty =[]'>remove</Btn>
			</header>
            <main class="p_4 flex flex_inline flex_wrap gap-x_4 gap-y_3">
                <InputText  class="flex_auto w_80" :value="''">Faculty Name</InputText>
                <InputText class="flex_auto"  :value="''">Personify ID</InputText>
                <InputText  class="flex_auto" :value="''">Location</InputText>
                <InputText  class="flex_auto" :value="''">Role</InputText>
            </main>
            </section>
            </ListLoader>
...
export default {
	...
	components: { Btn, InputText, InputTextArea, FieldSetGroup, ListLoader, ListEmptyCard, InputTextAndButton },
	methods: {
		newPresentation() { this.presentations=[{}];},
		attachPresentation() { },
		removePresentation() {
			this.presentations=[];
		}
